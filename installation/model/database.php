...
class InstallationModelDatabase extends JModelBase
{
	...
	public function populateDatabase($db, $schema)
	{
		...
		// Language
		$options = JFactory::getLanguage();
		$lang = strtolower(str_replace('-', '_', $options->getTag()));
		
		$buffer = str_replace('virtuemart_categories_en_gb', 'virtuemart_categories_' . $lang, $buffer);
		$buffer = str_replace('virtuemart_manufacturercategories_en_gb', 'virtuemart_manufacturercategories_' . $lang, $buffer);
		$buffer = str_replace('virtuemart_manufacturers_en_gb', 'virtuemart_manufacturers_' . $lang, $buffer);
		$buffer = str_replace('virtuemart_paymentmethods_en_gb', 'virtuemart_paymentmethods_' . $lang, $buffer);
		$buffer = str_replace('virtuemart_products_en_gb', 'virtuemart_products_' . $lang, $buffer);
		$buffer = str_replace('virtuemart_shipmentmethods_en_gb', 'virtuemart_shipmentmethods_' . $lang, $buffer);
		$buffer = str_replace('virtuemart_vendors_en_gb', 'virtuemart_vendors_' . $lang, $buffer);
		$buffer = str_replace('active_languages=[\"en-GB\"]', 'active_languages=[\"'.$options->getTag().'\"]', $buffer);
